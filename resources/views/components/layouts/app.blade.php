...
<head>
    ...
    <meta name="user-role"         content="{{ session('user_role', 'owner') }}">
    <meta name="tenant-profile-id" content="{{ session('tenant_profile_id', '') }}">
    <meta name="tenant-user-id"    content="{{ session('tenant_user_id', '') }}">
...
<div
    ...
    class="fixed top-4 {{ app()->getLocale() === 'ar' ? 'left-4' : 'right-4' }} z-[100] flex flex-col gap-2"
    x-cloak
>
...
@php
    ...
    $isAdmin  = in_array($role, ['super_admin', 'owner', 'admin']);
    ...
    $roleBadgeClass = match($role) {
        ...
        'owner'       => 'bg-violet-500 text-white',
...
<div class="flex h-full">
    ...
    <aside
        ...
        class="fixed inset-y-0 {{ app()->getLocale() === 'ar' ? 'right-0' : 'left-0' }} z-30 flex w-64 flex-col bg-[#00594F] transition-transform duration-200 ease-in-out lg:static lg:translate-x-0"
    >
        {{-- Brand --}}
        ...
        {{-- Tenant selector — only for multi-tenant roles --}}
        @if(!$isStudent && session('tenants') && count(session('tenants')) > 1)
        <div class="px-4 py-3 border-b border-white/10">
            ...
            <select
                onchange="document.getElementById('tenantIdInput').value=this.value; document.getElementById('tenantSwitchForm').submit()"
        ...
        @elseif(session('current_tenant.name'))
        <div class="px-4 py-3 border-b border-white/10">
            <p class="text-[#D9C89D] text-[10px] font-semibold uppercase tracking-wider mb-1">{{ __('app.tenant') }}</p>
            <p class="text-white text-sm font-medium truncate">{{ session('current_tenant.name') }}</p>
        </div>
        @endif
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            {{-- People — admins and teachers see students; admins also see teachers --}}
            ...
            <a href="{{ route('classes') }}" class="sidebar-link {{ request()->routeIs('classes') ? 'active' : '' }}">
                ...
                {{ $isStudent ? __('app.nav_my_classes') : __('app.nav_classes') }}
            </a>
            <a href="{{ route('enrollments') }}" class="sidebar-link {{ request()->routeIs('enrollments') ? 'active' : '' }}">
                ...
                {{ $isStudent ? __('app.nav_my_enrollments') : __('app.nav_enrollments') }}
            </a>
            <a href="{{ route('assignments') }}" class="sidebar-link {{ request()->routeIs('assignments') ? 'active' : '' }}">
                ...
                {{ $isStudent ? __('app.nav_my_assignments') : __('app.nav_assignments') }}
            </a>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded {{ $roleBadgeClass }} shrink-0">{{ $roleLabel }}</span>
                    </div>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="h-16 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center gap-4 px-4 lg:px-6 shrink-0">
            ...
            <button
                ...
                :title="isDark ? '{{ __('app.light_mode') }}' : '{{ __('app.dark_mode') }}'"
            >
                ...
                <svg x-show="!isDark" class="w-4.5 h-4.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
