    ...
    <div x-show="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="modal-panel max-w-sm" @click.stop>
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold text-slate-900 mb-1">{{ __('app.confirm_delete') }}</h3>
                <p class="text-sm text-slate-500 mb-6">{{ __('app.confirm_delete_msg') }}</p>
                <div class="flex gap-3">
                    <button @click="showDeleteConfirm = false" class="btn-secondary flex-1 justify-center">{{ __('app.cancel') }}</button>
                    <button @click="remove()" class="btn-danger flex-1 justify-center">{{ __('app.delete') }}</button>
                </div>
            </div>
        </div>
    </div>
