...
<div x-data="teachersPage()" x-init="init()" x-cloak>
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle" x-text="`${items.length} staff members`"></p>
        </div>
    ...
    <div class="flex gap-3 mb-4">
        <div class="relative flex-1 max-w-xs">
            ...
            <input x-model="search" type="search" placeholder="Search teachers…" class="form-input pl-9">
        </div>
    </div>
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
            ...
            <div class="modal-panel" @click.stop>
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <h3 class="font-semibold text-slate-900" x-text="editingId ? 'Edit Teacher' : 'Add Teacher'"></h3>
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <input x-model="form.name" type="text" class="form-input" placeholder="Dr. Sara Al-Mousa">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label class="form-label">Email</label>
                            <input x-model="form.email" type="email" class="form-input" placeholder="teacher@school.edu">
                        </div>
                        <template x-if="!editingId">
                            <div class="col-span-2 sm:col-span-1">
                                <label class="form-label">Password</label>
                                <input x-model="form.password" type="password" class="form-input" placeholder="min. 8 characters">
                            </div>
                        </template>
                        <div class="col-span-2 sm:col-span-1">
                            <label class="form-label">Employee ID</label>
                            <input x-model="form.employee_id_number" type="text" class="form-input" placeholder="EMP-001">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label class="form-label">Specialization</label>
                            <input x-model="form.specialization" type="text" class="form-input" placeholder="Mathematics">
                        </div>
                        <div class="col-span-2">
                            <label class="form-label">Bio</label>
                            <textarea x-model="form.bio" rows="3" class="form-textarea" placeholder="Brief biography…"></textarea>
                        </div>
                    </div>
                </div>
                <div class="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
                    <button @click="showModal = false" class="btn-secondary">Cancel</button>
                    <button @click="save()" class="btn-primary" x-text="editingId ? 'Save Changes' : 'Create Teacher'"></button>
                </div>
            </div>
        </div>
    </template>
